...
import com.junjunguo.phialmaps.geocoding.GeocoderLocal;
import com.junjunguo.phialmaps.model.MyMap;
...
public class Variable {
    ...
    private Variable() {
        ...
        this.smoothON = true;
        this.mapDirectory = "phialmap/maps/";
        this.dlDirectory = "phialmap/downloads/";
        this.trackingDirectory = "phialmap/tracking/";
    ...
    public int getMapStyleIndex() { return mapStyleIndex;}
    public void setMapStyleIndex(int mapStyle) {this.mapStyleIndex = mapStyle;}
    ...
    public boolean loadVariables(VarType varType)
    {
      ...
      {
        content = readFile("phialmapsavedfile.txt");
        loadStatus.put(VarType.Base, Boolean.TRUE);
      }
      else
      {
        content = readFile("phialmapsavedfile_" + varType + ".txt");
      ...
      try
      {
        ...
        if (varType == VarType.Base)
        {
            setTravelMode(TravelMode.valueOf(toUpperFirst(jo.getStr("travelMode", TravelMode.Car.toString()))));
            ...
            setMapStyleIndex(jo.getInt("mapStyleIndex", 1));
            setImperalUnit(jo.getBool("isImperalUnit", false));
            setSmoothON(jo.getBool("smoothON", true));
    ...
    public boolean saveVariables(VarType varType) {
        ...
        try
        {
          if (varType == VarType.Base)
          {
            ...
            jo.put("mapStyleIndex", getMapStyleIndex());
        ...
        if (varType == VarType.Base)
        {
          return saveStringToFile("phialmapsavedfile.txt", jo.toString());
        }
        else
        {
          return saveStringToFile("phialmapsavedfile_" + varType + ".txt", jo.toString());
        }
    }
    ...
    {
      return f.startsWith("phialmapsavedfile") && f.endsWith(".txt");
    }
