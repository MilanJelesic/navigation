...
public class TargetDirComputer
{
  ...
  public GHResponse createTargetdirResponseFromArray(List<GHPoint> points)
  {
    GHResponse resp = new GHResponse();
    PathWrapper pr = new PathWrapper();
    PointList pl = new PointList();
    ...
    for(int i=0;i<points.size();i++) {
      pl.add(points.get(i).lat, points.get(i).lon);
      if(i+1 < points.size()) {
        distance += GeoMath.fastDistance(points.get(i).lat, points.get(i).lon, points.get(i + 1).lat, points.get(i + 1).lon);
      }
    }
//    pl.add(toLat, toLon);
    pr.setPoints(pl);
