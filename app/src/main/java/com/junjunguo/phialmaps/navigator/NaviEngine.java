...
import com.junjunguo.phialmaps.util.GeoMath;
import com.junjunguo.phialmaps.util.LightSensor;
...
import com.junjunguo.phialmaps.util.SpeedUtil;
...
public class NaviEngine
{
  ...
  public void setNavigating(Activity activity, boolean active)
  {
    ...
    if (active == false)
    {
      ...
      MapHandler.getMapHandler().setCustomPointIcon(activity, R.drawable.ic_my_location_dark_36dp);
      if (pos != null)
      {
    ...
    MapHandler.getMapHandler().setCustomPointIcon(activity, R.drawable.ic_navigation_black_36dp);
  ...
  private void initFields(Activity activity)
  {
    ...
    navbottom_arrive = activity.findViewById(R.id.navtop_arrive_time);
    ...
    navspeed_unit = activity.findViewById(R.id.speed_unit);
    navspeed_unit.setText((Variable.getVariable().isImperalUnit())?"mph":"kph");
    speedUtil.initTextView(navspeed_text, activity.findViewById(R.id.speed_sign_layout));
  }
  ...
  public void setMapUpdatesAllowed(Context appContext, boolean allowed)
  {
      if (this.mapUpdatesAllowed != allowed)
      {
        ...
        if (allowed)
        {
          ...
          MapHandler.getMapHandler().setCustomPointIcon(appContext, R.drawable.ic_navigation_black_36dp);
        }
        else
        {
          MapHandler.getMapHandler().resetTilt(0);
          MapHandler.getMapHandler().setCustomPointIcon(appContext, R.drawable.ic_my_location_dark_36dp);
        }
      }
  }
  ...
  {
    if (active == false) { log("active false");return; }
    if (uiJob == UiJob.RecalcPath) { log("uijob error");return; }
      ...
      protected void onPostExecute(NaviInstruction in)
      {
        if (in == null)
        {
          if (NaviEngine.this.uiJob == UiJob.RecalcPath)
          {
            ...
            {
              instructions = null;
              MapHandler.getMapHandler().calcPath(recalcFrom.getLatitude(), recalcFrom.getLongitude(), recalcTo.getLatitude(), recalcTo.getLongitude(), activity, true);
  ...
  private void showInstruction(NaviInstruction in)
  {
    ...
    else if(nearestP.isDirectionOk())
    {
      ...
      navtop_nextloc.setText(in.getNextStreet());
    ...
    navbottom_arrive.setText(Navigator.getNavigator().getDistance() + "  " + Navigator.getNavigator().getArriveTime());
